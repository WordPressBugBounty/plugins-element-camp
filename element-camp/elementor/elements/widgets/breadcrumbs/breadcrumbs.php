...
/**
 * Helper function to get term link with caching support
 */
if (!function_exists('elementcamp_cached_get_term_link')) {
    function elementcamp_cached_get_term_link($term_id, $taxonomy) {
        // You can implement caching here if needed
        return get_term_link((int)$term_id, $taxonomy);
    }
}
 ...
 */
if (!function_exists('elementcamp_get_primary_category_id')) {
    function elementcamp_get_primary_category_id() {
        $categories = get_the_category();
        return !empty($categories) ? $categories[0]->term_id : false;
    }
}

/**
 * Creates breadcrumb navigation for WordPress
 *
 * @param string $separator The separator to use between breadcrumb items
 * @return void
 */
function elementcamp_breadcrumbs($separator = '', $pages_text) {
    # breadcrumbs ----------

    $home_text = (is_rtl()) ? esc_html__('الصفحات', 'element-camp') : esc_html__('Pages', 'element-camp');
    if(isset($pages_text) && !empty($pages_text)) {
        $home_text = $pages_text;
    }
    ...
    if (!is_home() && !is_front_page() || is_paged()) {
        ...
        if (is_category()) {
            ...
            if ($category->parent !== 0) {
                ...
                foreach ($parent_categories as $parent_category) {
                    $breadcrumbs[] = array(
                        'url' => elementcamp_cached_get_term_link($parent_category, 'category'),
        ...
        elseif (is_singular()) {
            ...
            if (get_post_type() == 'post') {
                ...
                $category = $category ? get_category($category) : null;
                ...
                if (!empty($category)) {
                    ...
                    $breadcrumbs[] = array(
                        'url' => elementcamp_cached_get_term_link($category->term_id, 'category'),
...
class ElementCamp_Breadcrumbs extends Widget_Base {
	...
	protected function register_controls() {
        ...
        $this->add_responsive_control(
            'breadcrumb_padding',
            [
                'label' => esc_html__('Breadcrumb Padding', 'element-camp'),
                'type' => Controls_Manager::DIMENSIONS,
                'size_units' => ['px', '%', 'em', 'rem', 'custom'],
                'selectors' => [
                    '{{WRAPPER}} .tcgelements-breadcrumbs .breadcrumb' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
        );
        ...
        $this->add_group_control(
            ...
            [
                ...
                'selector' => '{{WRAPPER}} .tcgelements-breadcrumbs span,{{WRAPPER}} .tcgelements-breadcrumbs a',
            ]
        );
    ...
    protected function render() {
        ...
        <div class="tcgelements-breadcrumbs">
            <div class="path">
                <?php elementcamp_breadcrumbs($separator, $settings['custom_text_for_pages']); ?>
            </div>
        </div>
