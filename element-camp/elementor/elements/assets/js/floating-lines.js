(function ($) {
    "use strict";
    /**
     * FloatingLines Vanilla JS Implementation
     * Converted from React version by Antigravity
     */
...
const vec3 PINK  = vec3(20.0, 71.0, 245.0) / 255.0;
...
`;
    ...
    class FloatingLines {
        constructor(container, options = {}) {
            ...
            // Bound references for clean removal on destroy
            this._boundDocMove = (e) => this._onDocumentPointerMove(e);
        ...
        init() {
            ...
            // ── Key fix: listen on document so z-index never blocks events ────
            // We then translate clientX/Y into canvas-local coords ourselves.
        ...
        /**
         * Called on every document pointermove.
         * Converts page coordinates → canvas-local coordinates.
         * If the cursor is outside the canvas rect, smoothly fade influence out.
         */
        _onDocumentPointerMove(event) {
            ...
            if (!inside) {
                // Cursor outside canvas — fade out
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/tcgelements-canvas.default', function ($scope) {
            const $container = $scope.find('.floating-lines-container');
            if (!$container.length) return;

            const el = $container[0];
            if (el.classList.contains('fl-initialized')) return;
            el.classList.add('fl-initialized');

            new FloatingLines(el, {
