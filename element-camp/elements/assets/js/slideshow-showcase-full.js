(function ($) {
    "use strict";
    function elementcamp_slideshow_showcase_full($scope, $) {
        {

            // background image
            var pageSection = $(".tcgelements-slideshow-showcase .bg-img");
            ...
            <path fill="${this.settings.frameFill}" d="${this.paths.initial}"/>`;
                    ...
                    initEvents() {
                        ...
                        this.DOM.el.addEventListener('wheel', (event) => {
                            if (event.deltaY > 0) {
                                this.navigate('next');
                            } else {
                                this.navigate('prev');
                ...
                if ($scope.find('.full').length > 0) {
                    new Slideshow(document.querySelector('.tcgelements-slideshow-showcase.full'));
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/tcgelements-slideshow-showcase.default', elementcamp_slideshow_showcase_full);
    });
